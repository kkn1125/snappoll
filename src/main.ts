...
async function bootstrap() {
  ...
  app.useStaticAssets(path.join(path.resolve(), 'src', 'views', 'public'));
  app.setBaseViewsDir(path.join(path.resolve(), 'src', 'views', 'pages'));
  // 템플릿 엔진 설정
  app.engine(
    'hbs',
    exphbs.engine({
      extname: 'hbs',
      layoutsDir: path.join(path.resolve(), 'src', 'views', 'pages', 'layouts'),
